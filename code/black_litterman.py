...
class BlackLitterman:
    ...
    def __init__(self):
        ...
        self.realistic_optimization = REALISTIC_OPTIMIZATION
    ...
    def normalize_weight(self, weight):
        ...
        total = weight.sum()
        if abs(total) < 1e-12:
            return np.ones(len(weight)) / len(weight)
        return weight / total

    def regularize_covariance(self, stock_cc_ret):
        cov = np.array(stock_cc_ret.cov())
        if self.cov_shrinkage > 0:
            diag_cov = np.diag(np.diag(cov))
            cov = (1 - self.cov_shrinkage) * cov + self.cov_shrinkage * diag_cov
        cov = cov + np.identity(cov.shape[0]) * self.cov_ridge
        return cov
    ...
    def get_market_value_weight(self):
        ...
        total_col = next((c for c in mv.columns if str(c).lower() == "total"), None)
        if total_col is None:
            raise ValueError("Market value file must contain a Total column.")
        stock_names = [c for c in mv.columns if c != total_col]
        # calculate market value weight
        for n in stock_names:
            mv[n] = mv[n] / mv[total_col]
        # 去掉第一列，与收益率文件保持数量一致
        mv = mv.drop(index=[0])
    ...
    def get_implied_excess_equilibrium_return(self, stock_cc_ret, w_mkt):
        ...
        rf = np.log(1 + self.risk_free_rate_annual) / self.periods_per_year
        mkt_cov = self.regularize_covariance(stock_cc_ret)
    ...
    def get_posterior_combined_return(self, implied_ret, mkt_cov, P, Q, omega):
        ...
        k = np.linalg.pinv(inv_tau_cov + np.dot(np.dot(P.T, inv_omega), P))
        posterior_ret = np.dot(k, np.dot(inv_tau_cov, implied_ret) +
                            np.dot(np.dot(P.T, inv_omega), Q))
        return posterior_ret

    # Step10. 计算由BL模型得到的新权重weight_bl
    def get_weight_bl(self, posterior_ret, mkt_cov, lambd, previous_weight=None):
        if not self.realistic_optimization:
            weight_bl = np.dot(np.linalg.pinv(lambd * mkt_cov), posterior_ret)
            return self.normalize_weight(weight_bl)

        n_assets = len(posterior_ret)
        if previous_weight is None:
            previous_weight = np.ones(n_assets) / n_assets
        previous_weight = self.normalize_weight(previous_weight)

        def objective(weight):
            expected_utility = np.dot(weight, posterior_ret) - 0.5 * lambd * np.dot(np.dot(weight, mkt_cov), weight.T)
            turnover_cost = self.turnover_penalty * np.sum(np.abs(weight - previous_weight))
            return -expected_utility + turnover_cost
        ...
        if self.turnover_limit is not None:
            constraints.append({
                "type": "ineq",
                "fun": lambda w: self.turnover_limit - np.sum(np.abs(w - previous_weight))
            })
        ...
        return self.normalize_weight(result.x)

    # Step11. 计算指定时间窗口T和不同Views下得到的BL模型新权重、真实收益率
    def get_post_weight(self, start_idx, previous_weight=None):
        ...
        mkt_cov = self.regularize_covariance(stock_cc_ret)                  # 将T区间部分的股票收益率计算成协方差矩阵
        ...
        if (view_type == 0):
            ...
            weight_bl = self.normalize_weight(mv_i)
        elif (view_type == 1 or view_type == 2 or view_type == 3):
            # weight_type == 1: 根据Views的类型，计算BL模型得到的新权重weight_bl
            weight_bl = self.get_weight_bl(posterior_ret, mkt_cov, lambd, previous_weight)
    ...
    def calculate_comparative_return(self, start_idx, end_index):
        ...
        stock_cc_ret = self.stock_cc_ret.copy()                                 # 传入10只股票收益率
